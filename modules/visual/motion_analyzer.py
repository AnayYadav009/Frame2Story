...
def sample_frames(video_path, start_sec, end_sec, fps, sample_step_sec=1.0):
    frames = []
    ...
    if not frames and end_sec >= start_sec:
        try:
            frames.append(get_frame_at_index(video_path, time_to_frame(end_sec, fps)))
        except Exception:
            pass
        
    return frames

def compute_scene_motion(video_path, scene, fps, sample_step_sec=1.0):
    
    start = scene["start"]
    end = scene["end"]
    
    frames = sample_frames(video_path, start, end, fps, sample_step_sec=sample_step_sec)
